...
void draw_chip(GLuint program, GLuint colorLoc, float* color, GLuint radiusLoc, float radius, GLuint centerLoc, float* center, GLuint VAO){
    glUseProgram(program);
    glBindVertexArray(VAO);
...
int main() {
    ...
    GLuint colorLocNormal = glGetUniformLocation(shaderProgramNormal, "sq_color");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        draw_panel((float)marginLeft, (float)y1 + y2 / 4, rightBorder, y2 / 2, turn, board);
        ...
        for (int i = 0; i < SIZE; i++){
            for (int j = 0; j < SIZE; j++){
                ...
                if (board[i][j] != clear)
                    draw_chip(shaderProgramChip, colorLocChip, chip_colors[board[i][j]], radiusLoc, CHIP_RADIUS, centerLoc, current_chip_center, VAO_CI);
            
                if (state == PLAYER_MOVE && is_valid_move(board, i, j, turn % 2))
                        draw_chip(shaderProgramChip, colorLocChip, suggestion_color, radiusLoc, SUGGEST_RADIUS, centerLoc, current_chip_center, VAO_CI);        
            }
        }
